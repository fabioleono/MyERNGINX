...
const verifyToken = (req, res, next) => {
  ...
  jwt.verify(token, process.env.KEY_SECRET, (error, decoded) => {
    ...
    req.userId = user;
    req.family = family;
    console.log("userToken ", user," family ",family," iatToken ",iat,"expToken ",exp);
    ...
    if(url.includes(family)===false) {
      ...
      return res
        .status(403)
        .json({ sucess: false, status: "Token Authentication Credentials Failed" });
    }
    ...
    modelProfile.verifyUser(user, (err, data) => {
      ...
      console.log("ENV ", process.env.NODE_ENV);
      if (req.sessionID !== data[0].session && process.env.NODE_ENV==='production') {
        req.session.destroy();
        res.clearCookie("connect.sid");
